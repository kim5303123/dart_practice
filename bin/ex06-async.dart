...
import 'dart:async';
import 'dart:convert';  //  json convert를 위한 라이브러리 import

String jsonString = '{"name": "홍길동", "age": 25, "isThief": true}';
Future<Map<String, dynamic>> fetchUserData() async {
  await Future.delayed(Duration(seconds: 2)); //  2초 ㄱㄷ
...
Stream<int> counterStream(int max) async* { //  async*  ->  Generator 표시
  for (int i = 1; i <= max; i++) {
    await Future.delayed(Duration(seconds: 2)); //  1초 ㄱㄷ
...
//  StreamController: Stream을 생성하고 데이터를 추가하는 열할 (Publisher)
